...
The **host** end of the paired host/client tools: the self-contained binary the user runs on the machine whose Chrome is shared (and can delete anytime). It:
...
## Layout
- `cmd/host/main.go` — cobra/fang CLI + orchestration (consent → launch → ephlink.Expose → teardown).

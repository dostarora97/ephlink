...
- **Consumers:** `host` (Go, Chrome-side) and `client` (Go, operator-side). Both are thin consumers of `ephlink`.
...
- [ ] **D10c** — Live "connected / being-controlled" indicator on the host (+ a separate consent step for active control vs. passive observe, revisiting the two-tier gate in D5).
...
- [ ] **D7** — Tightened least-privilege ACL grant (host reachable only by the client, only on the CDP port).
...
- **Ceiling operator:** a non-technical **end-user** — the host must be runnable by someone following simple instructions. This subsumes supervised/technical operators, so we design to the hardest case and get the easier ones for free.
- **Trust posture:** **mutually untrusted**, but via *modern sane defaults*, NOT a bespoke security apparatus. Guiding rule: prefer boring, well-audited primitives over custom crypto/validation. What "sane default" means concretely (so it isn't relitigated per component):
  ...
  - **Protect the operator from a hostile/compromised host:** treat inbound CDP as untrusted input — don't `eval`/execute host-supplied strings on the operator host; cap message sizes; validate JSON shape at the boundary; run least-privilege. Do NOT build custom attestation, host binary remote-verification, or anomaly detection — premature. A signed binary (later) covers "is this the real host."
...
- **Known tension accepted + mitigation:**
  ...
  - "User joins a vendor tailnet" ask → ephemeral node + tight ACL (host tag can reach only the client node, only on the CDP port) + clear consent copy. It's a single-purpose, single-peer, expiring node, not a general tailnet member.
...
- **Fresh temp profile = hard default.** The host spawns Chrome with a throwaway `--user-data-dir`; the user signs in fresh inside it. Smallest blast radius, clean teardown (delete the temp dir), matches the ephemeral theme.
...
- **Remote debugging is guaranteed on, not assumed.** Because the host launches its own Chrome with `--remote-debugging-port`, the debug port is always enabled for the built path — the `chrome://inspect/#remote-debugging` toggles are irrelevant here. A running Chrome the tool did NOT launch cannot be retro-enabled: `chrome://inspect` only *discovers/forwards* to targets that already expose a port (e.g. remote Android/other-host targets); it does not turn on a debug port for the browser you are viewing. This is the fundamental reason the "attach to running Chrome" case below is hard. The host preflights the port (see D15) so a port already held by another/your-own Chrome fails loud immediately with a clear message, rather than surfacing as a confusing timeout.
...
- **Direction: operator-generated.** The operator mints the credential (holds the Tailscale API creds); the user just runs the host with a key. Fits the non-technical ceiling.
- **Phasing:**
  - **Now:** the operator mints a raw ephemeral, single-use, pre-authorized, tagged, short-TTL auth key and passes it to the user; the host joins with it directly.
    - Caveat (accepted for supervised use only): a raw auth key is a bearer credential; if leaked before use it could authorize a tailnet join. Mitigated by single-use (consumed on first join), short TTL, and tag scoping (can only reach the client's CDP port). Deliver over a private channel; treat as a secret.
  - **Later (committed):** the operator mints a short opaque **pairing token**; the host redeems it at a minimal **provisioning endpoint** that mints the real ephemeral key server-side. The sensitive key never transits human channels; the endpoint becomes the control point for consent-record, audit, TTL, and revocation.
- **Design invariant that makes the phasing free:** the host takes an **opaque key/token** — the host's code and UX are identical across phases; only the operator/provisioning side changes.
...
- **Connection model: client → host.** Chrome serves the debug port on the host side; the client connects into it (the natural CDP model). The host's `tsnet` binds the CDP port to the tailnet so only the client tag can reach it. The ephemeral key is minted with the host tag.
  - **Risk + fallback:** some host-side networks are egress-only and may block inbound even over the mesh → if that happens in the field, fall back to host-dials-client (host opens the connection outbound; client becomes the rendezvous). The transport-agnostic seam makes this swap local.
...
- The strict host→client-only grant is currently optional (the default "allow" ACL already permits the traffic); it's listed under Deferred hardening.
...
- **Support all stop triggers.** Primary: user-quit (Ctrl-C / close / Stop) and client-disconnect. TTL-expiry is the safety net. Also handle network drop. Note: deleting the binary mid-run does NOT reliably kill a running process; the real kill is quit/Ctrl-C, which is made bulletproof.
...
- **"Fix if it bites" (no work now):** DERP/UDP fallback (automatic), host-side inbound blocking (documented host-dials-client fallback), exotic network conditions.
...
- **Transport = Tailscale, not us.** `tsnet.Server.Dial(ctx, "tcp", "<host>:<port>")` on the client yields a raw `net.Conn` straight to the host's Chrome; the host exposes its port via `tsnet.Listen`. No relay, no hand-written byte-pipe.
...
- **Language unified on Go.** The client became stdlib-ReverseProxy-over-`tsnet`, so it's naturally Go and embeds `tsnet` directly — superseding an earlier Deno/TS client prototype (kept only as a reference until the Go path was proven, then removed). The local `ws://localhost:PORT` output is unchanged, so all CDP clients still attach.
...
- First proved the full path with `tailscale serve --tcp` (zero hand-rolled transport): a CDP client → client (CDP rewrite) → Tailscale → the host's Chrome, all working; the transport-agnostic seam meant loopback→tailnet was a one-flag change.
- Then moved to **embedded `tsnet`** (no dependency on a system Tailscale install on the user's machine — the "download → run → accept" UX), and proved it end-to-end on a real tailnet: mint an ephemeral tagged key via an OAuth client → the host joins as an ephemeral node with no system Tailscale → the client dials it → Playwright drives Chrome → Ctrl-C tears down Chrome + temp profile and the ephemeral node auto-deregisters. The `--serve` (system-Tailscale) path was subsequently dropped in favor of embedded-only.

### D19 — Extraction: the ephemeral link is the reusable thing; CDP is a consumer

Recognizing that what was built is really "an ephemeral secure connection between two machines, with CDP as a special case on top," the transport was extracted into **`ephlink`** — its own CDP-agnostic Go module with a **symmetric API** (`Join` → `Expose`/`Dial`/`Serve` as node capabilities; no privileged side). `host` and `client` became thin consumers; `mint` is a thin CLI over `ephlink.Mint`. `Dial` resolves a peer name → tailnet IP via online peers for robust MagicDNS. The extraction test: `ephlink`'s public API mentions nothing about CDP/Chrome/HTTP — so the same library can carry SSH, a database port, a dev server, or any TCP service.

### D20 — Multi-host: generic host + operator-side re-present + tailnet-as-registry

Extending from one host to a fleet, without sacrificing the generic core. Key decisions:

- **`host` stays generic; the CDP transform is operator-side.** Each `host` does a *raw* mesh expose of its Chrome CDP port (knows nothing about CDP). The operator's `client serve-cdp` dials the raw host over the mesh, applies the CDP `Host`/`webSocketDebuggerUrl` rewrite, and re-presents it. This keeps `host` reusable for any TCP service, and lets the CDP logic be fixed/updated in one operator-side place without redeploying host binaries. (This reverted an earlier idea of moving the rewrite into `host` — that got per-host tailnet names for free but made `host` CDP-specific; the re-present approach gets both.)
- **Each host is its own named tailnet node** — naming, isolation, and scoping come from the mesh, not from local ports or `/etc/hosts`. The raw host joins as `cdp-host-<name>-src`; the client presents `cdp-host-<name>`. Consumers attach to `http://cdp-host-<name>.<tailnet>.ts.net:<port>`. Two nodes per host, accepted for the clean namespace + generic host.
- **The tailnet is the registry — no local database.** `client list`/`remove` read/act via the Tailscale API filtered by `tag:ephlink-host`; the key `description` carries the host↔name association. The daemon is stateless and re-derives the fleet from the tailnet on every run. (A SQL/local store was considered and rejected as drift-prone and unnecessary.)
- **Connection establishment + persistence are Tailscale's job** (NAT traversal, DERP fallback, keepalive, reconnect, roaming). We write none of it; our lifecycle choice is only ephemeral-vs-persistent nodes.
- **Dynamic add, not static config.** Hosts aren't known up front, so the model is `client add <name>` (mint on demand, print the `host` command for out-of-band handoff) rather than a static TOML file. The minted key must reach the host's *other* machine — bridged manually now (mint + print); a provisioning endpoint / pairing token (D6) is the deferred upgrade.
- **Generic library primitives added:** `ephlink.ListenOnMesh` (raw mesh listener for L7 servers) and `ServeOnMesh` (mesh-side per-conn relay with optional transform) — both CDP-agnostic; the CDP rewrite lives in `internal/cdp` and is injected as the upstream dialer + handler by `client`.
- **`cmd/host` remains CDP-specialized (YAGNI):** the generic capability lives in `ephlink`; a future non-CDP exposer is a trivial, non-breaking addition. Not built until a real use appears.
